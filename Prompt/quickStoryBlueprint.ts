...
2. Each chapter needs a summary of 45-70 words naming its primary dramatic scene and how it serves the arc (establishing, revealing prior cause, deepening, settling weight, crisis, or resolution) — not a chain of events. The seven summaries together must form a shaped arc, not seven escalating pressure steps. Each chapter also requires a time_context field: a brief natural-language note on when it takes place relative to the previous chapter (e.g. "story opens", "immediately follows", "days later", "weeks later") — this is the mechanism that gives the story its temporal rhythm, so vary it intentionally.
...
2. 每章必须提供 summary（60-80字），说明本章的主要戏剧场景及其在弧线中的职能（建立/揭示前因/深化/沉淀/危机/收束），而非罗列事件序列。七条 summary 顺读下来应呈现有形状的弧线，而不是七遍「压力升级」的斜面。每章还必须填写 time_context 字段：一句简短的时间关系描述，说明本章相对上一章的时间位置（如「故事开始」「紧接上章」「数日后」「数周后」）——这是为故事建立时间节奏的核心机制，请刻意安排变化，不要每章都是「紧接上章」。
