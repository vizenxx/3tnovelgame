...
1a. FIRST design arc_design — the macro shape of the whole seven-chapter arc, BEFORE writing the per-chapter summaries: how much total time the story spans (a night, weeks, years — your choice), how the rhythm is distributed (which chapters sit close together, where time leaps so change can settle), and how the seven functions string into a shaped arc rather than a linear pressure ramp. Every chapter's summary and time_context must then be DERIVED from this macro design — the chapters realize the arc, they do not each improvise locally.
...
1a. 先设计 arc_design——在写各章 summary 之前，先定整条 7 章弧线的宏观形状：这个故事总共跨越多长时间（一夜、数周、数年皆可由你定）、节奏如何分布（哪几章贴得近、哪几处让时间跳过一段好让变化沉淀）、七种职能如何串成一条有起伏的弧而非线性升压。之后每一章的 summary 与 time_context 都必须从这个宏观设计「推导」出来——各章是去实现这条弧线，而不是各自就地即兴。
