...
2. 每章必须提供 summary（60-80字），说明本章的主要戏剧场景、在弧线中的职能（建立/揭示前因/深化/沉淀/危机/收束），以及这章的叙事质地——它是缓慢省思的、急迫紧张的、还是内向亲密的。这个质地维度告诉章节生成器它要创造的是什么体验，而不只是哪些事件。不要罗列事件序列。七条 summary 顺读下来应呈现有形状的弧线，而不是七遍「压力升级」的斜面。每章还必须填写 time_context 字段：一句简短的时间关系描述，说明本章相对上一章的时间位置（如「故事开始」「紧接上章」「数日后」「数周后」）——这是为故事建立时间节奏的核心机制，请刻意安排变化，不要每章都是「紧接上章」。
