...
export default async function (req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const prompt = `你是一个互动小说世界构建师。
      ...
      6.2 **条件约束极其严格**：\`condition_char\` 必须且只能填入上方创建的角色ID（必定是 c1, c2..）；\`condition_chapter\` 必须处于 2 到 6 的整数；\`condition_action\` 必须且只能从 "bless" 与 "curse" 二选一；\`desc\` 字段务必描绘出该支线发生时的具体“过场剧情变动和隐藏内幕”。
